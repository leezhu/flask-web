...
var app = angular.module('vedioApp',[]);

app.controller('vedioController',function($scope,$http,$location){
	//
	var table_data;
	...
	$(document).ready(function(){	
		$('#tablelist').bootstrapTable({
		  url: window.location.origin+"/watch/queryAllVedio",
		  ...
		  onDblClickCell: function (field, value,row,td) {
           //alert(row['vedio_name']);
					...
					$http.post($location.path()+'/watch/queryVedio',{'vedio_id':row['vedio_id']}).then(function(resp){
	...
	$scope.deleteVedio = function(){

		var data = $('#tablelist').bootstrapTable('getSelections');		
		console.log(data);
		$http.post($location.path()+'/watch/deleteVedio',{'data':data}).then(function(resp){
	...
	$scope.editFunction = function(){
		$http.post($location.path()+'/watch/editVedio',$scope.data).then(function(resp){
				 alert("更新成功");

...
var app = angular.module('addCourseApp',['ngFileUpload']).config(function($httpProvider){
	$httpProvider.defaults.headers.common={'x-session-token':'nnnnnn'}
});

app.controller('addCourseController',function($scope,$http,$location,Upload){
		var formdata={};

	$scope.submit = function(){
		//data['']=$scope.course;课程路径
		formdata['course_type']=$scope.course_type;
		formdata['course_describe']=$scope.course_describe;
		formdata['course_name']=$scope.course_name;
		formdata['course_teacher']=$scope.course_teacher;
		formdata['course_price']=$scope.course_price;
		formdata['course_status']=1;
	...
	$scope.upload = function(file){
		...
		Upload.upload({
			url:$location.path()+'/watch/saveCourse',
